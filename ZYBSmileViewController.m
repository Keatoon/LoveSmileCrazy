...
#import "ZYBSmileViewController.h"
#import "ZYBDetailViewController.h"
#import "UMSocial.h"
#import "ZYBLoginViewController.h"

@interface ZYBSmileViewController ()<UMSocialUIDelegate>
@end

@implementation ZYBSmileViewController
...
-(void)oneDownLoadDataWithCount:(NSInteger) count{
    NSString *url=[NSString stringWithFormat:zSmileUrl,count];
...
{
    NSString *url=[NSString stringWithFormat:zSmileMoreUrl,time,count];
    __block typeof(self)blockself=self;
    [_manager GET:url parameters:nil success:^(AFHTTPRequestOperation *operation, id responseObject) {
        NSLog(@"下载成功");
        NSDictionary *dict=[NSJSONSerialization JSONObjectWithData:responseObject options:NSJSONReadingMutableContainers error:nil];
